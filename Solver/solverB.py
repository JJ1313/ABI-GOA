...
from Metaheuristics.GOA import iterarGOA
...
def solverB(id, mh, maxIter, pop, function, lb, ub, dim):
    ...
    # Genero una población inicial EEcontinua. Acá estamos resolviendo las funciones matemáticas benchmark.
    ...
    for iter in range(0, maxIter):
        ...
        if mh == "GWO":
            poblacion = iterarGWO(
                maxIter, iter, dim, poblacion.tolist(), fitness.tolist(), "MIN"
            )
        if mh == "WOA":
        ...
        if mh == "GOA":
            poblacion = iterarGOA(maxIter, iter, dim, poblacion.tolist(), Best.tolist(), fitness.tolist(), function)
        # calculo de factibilidad de cada individuo y calculo del fitness inicial
        for i in range(poblacion.__len__()):
            for j in range(dim):
                poblacion[i, j] = np.clip(poblacion[i, j], lb[j], ub[j])
    ...
    binary = util.convert_into_binary(
        dirResult + mh + "_" + function + "_" + str(id) + ".csv"
    )
